...
func TestExamplesComplete(t *testing.T) {
    ...
    cloudtrailS3BucketId := terraform.Output(t, terraformOptions, "dummy_trial_s3_bucket_id")
    // Verify we're getting back the outputs we expect
    assert.Equal(t, "am-euw1-bk-poc-test-s301-epd", cloudtrailS3BucketId)

    // Run terraform output to get the value of an output variable
    cloudtrailId := terraform.Output(t, terraformOptions, "dummy_trail_id")
    // Verify we're getting back the outputs we expect
    assert.Equal(t, "am-euw1-bk-poc-test-trail01-epd", cloudtrailId)
}
