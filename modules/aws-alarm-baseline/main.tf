...
resource "aws_cloudwatch_metric_alarm" "security_group_changes" {
  ...
  alarm_name                = "SecurityGroupEvents"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
  evaluation_periods        = "1"
...
resource "aws_cloudwatch_log_metric_filter" "s3_creation_deletion" {
  count = var.enabled && var.s3_creation_deletion ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "s3_creation_deletion" {
  count = var.enabled && var.s3_creation_deletion ? 1 : 0

  alarm_name                = "S3BucketActivityEventCount"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
  evaluation_periods        = "1"
  metric_name               = aws_cloudwatch_log_metric_filter.s3_creation_deletion[0].id
  namespace                 = var.alarm_namespace
  period                    = "300"
  statistic                 = "Sum"
  threshold                 = "1"
